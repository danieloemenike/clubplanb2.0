      ...
      <section className='w-full h-full flex  mt-4 py-10'>
        <div className='w-full h-full'>
          <div className='flex justify-center text-[1rem] md:text-[3rem] w-full h-full items-center px-2'>
            <h2 className='font-bold text-[2rem] lg:text-[4rem] font-gv bg-clip-text text-transparent bg-gradient-to-r  from-blue-400 via-white  to-white'>Regular Lounge Menu </h2>
          </div>
          <div className='w-full h-full'>
            ...
            {regularMenu.map((drinks: DrinkModel) => {
              ...
              if (!selectedCategory || (selectedCategory && drinks.category === selectedCategory && filteredDrinks.length > 0)) {
                return (
                  <div key={drinks.id}>
                    {filteredDrinks.length > 0 && <div className='  flex decoration-from-font justify-center'> <h3 className='font-bold text-[3rem] text-center mt-32 mb-12 font-gv capitalize tracking-[.20rem] bg-clip-text text-transparent bg-gradient-to-r  to-white  from-blue-400 '>{drinks.category}</h3> </div>}
                      ...
                      {filteredDrinks.map((drink) => (
                        <div key={drink.id} id={drink.id.toString()} className='rounded-md h-80 w-50 p-2 shadow   backdrop-filter backdrop-blur-lg '>
                          ...
                          <div className='w-full flex justify-center items-center  h-[17%] mb-4 py-8 px-3 border border-black drop-shadow-2xl rounded-xl'>
                            <h2 className=' text-base text-blue-300'> <span className='bg-clip-text '>{ drink.name } - </span><span className='rounded-md text-sm bg-white text-black p-1 font-semibold'>{ drink.price}</span></h2>
                          </div>
                        </div>
                      ))}
                    </div>
