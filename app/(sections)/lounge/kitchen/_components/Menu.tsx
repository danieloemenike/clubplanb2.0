      ...
      <section className='w-full h-full flex  mt-4 py-10'>
        <div className='w-full h-full'>
          <div className='flex justify-center text-[1rem] md:text-[3rem] w-full h-full items-center px-2'>
            <h2 className='font-bold text-[2rem] lg:text-[4rem] font-gv bg-clip-text text-transparent bg-gradient-to-r  from-red-400 via-white  to-white'>The Kitchen Menu </h2>
          </div>
          <div className='w-full h-full'>
            <div className="flex flex-col md:flex-row justify-center m-4 p-4 gap-2 md:gap-4">
              <select title="categories" onChange={handleCategoryChange} className='mt-4 p-2  bg-black rounded-md '>
                <option value='' className=''>All Meals</option>
                {kitchenMenu.map((food: DrinkModel) => (
              ...
              <input
                type='text'
                placeholder='Search for meals here...'
                value={searchTerm}
            ...
            {kitchenMenu.map((food: DrinkModel) => {
              const filteredDrinks = food.food.filter(
                (drink) =>
                  (!searchTerm || drink.name.toLowerCase().includes(searchTerm)) ||
                  (!searchTerm || drink.price.toLowerCase().includes(searchTerm))
              );

              if (!selectedCategory || (selectedCategory && food.category === selectedCategory && filteredDrinks.length > 0)) {
                return (
                  <div key={food.id}>
                    {filteredDrinks.length > 0 && <div className='  flex decoration-from-font justify-center'> <h3 className='font-bold text-[3rem] text-center mt-32 mb-12 font-gv capitalize tracking-[.20rem] bg-clip-text text-transparent bg-gradient-to-r  to-white  from-red-400 '>{food.category}</h3> </div>}
                      ...
                      {filteredDrinks.map((drink) => (
                        <div key={drink.id} id={drink.id.toString()} className='rounded-md h-80 w-50 p-2 shadow   backdrop-filter backdrop-blur-lg '>
                          <div className='h-60 w-full bg-white object-contain flex items-center justify-center overflow-hidden rounded-t-lg mb-2'>
                            <Image src={ drink.image } alt="food" width={ 290 } height={ 150} className='object-contain rounded-xl' loading='lazy'/>
                          </div>
                          <div className='w-full flex justify-center items-center  h-[17%] mb-4 py-8 px-3 border border-black drop-shadow-2xl rounded-xl'>
                            <h2 className=' text-base text-red-100'> <span className='font-medium'>{ drink.name } - </span><span className='rounded-md text-sm bg-white text-black p-1 font-semibold'>{ drink.price}</span></h2>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {kitchenMenu
              .flatMap((food: DrinkModel) => food.food)
              .filter(
                (drink) =>
                  (!searchTerm || drink.name.toLowerCase().includes(searchTerm)) ||
                  (!searchTerm || drink.price.toLowerCase().includes(searchTerm))
